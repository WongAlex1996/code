...
print WF "Id\tfutime\tfustat\tage\tgender\tgleason_score\tstage\tT\tM\tN\n";
foreach my $dir(@dirs){
	if(-d $dir){
	  ...
	  while(my $xmlfile=readdir(RD)){
	  	if($xmlfile=~/\.xml$/){
	  		#print "$dir\\$xmlfile\n";
				...
				my $gleason_score=$userxml->{$patient_key}{'shared_stage:stage_event'}{'shared_stage:gleason_grading'}{'shared_stage:gleason_score'}{'content'};  #G1/G2/G3
				...
				$gleason_score=(defined $gleason_score)?$gleason_score:"unknow";
				...
				my $survivalTime="";if($samp1e[5]>150){next;}
				...
				print WF "$patient_barcode\t$survivalTime\t$age\t$gender\t$gleason_score\t$pathologic_stage\t$pathologic_T\t$pathologic_M\t$pathologic_N\n";
			}
		}
		close(RD);
	}
